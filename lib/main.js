...
if (!webkit) {
    console.log('Failed to load webkit');
    process.exit();
}
...
Chimera.prototype.perform = function (options) {
    ...
    var run = 'with(' + JSON.stringify(locals) + '){(' + (options.run || function (callback) {
        callback(null, "");
    }) + ')(function(err,res) {window.chimera.callback(JSON.stringify(err), JSON.stringify(res));})}';
    var callback = options.callback || function (err, result) {
    };

    if (html && !url) {
        this.browser.renderSnippet(html, run, function (errStr, resStr) {
